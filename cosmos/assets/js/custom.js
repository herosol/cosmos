...
$(document).ready(function() {
    ...
    $(document).on('change', '#uploadFiles', function() {
        ...
        let bx = `#char${character}`;
        let elem = $(`${bx} .uploadBar`);
        let progressBar = $(`${bx} .uploadBar>span`)[0];
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                if (rs.upload_status == 1) {
                    $.each(rs.image, function(i, img) {
                        $(`${bx} ul.imgLst`).append(
                            `<li><input type="hidden" name="images[${character}][]" value="${img}"><div class="image"><img src="${rs.image_path[i]}" alt=""><div class="closeBtn"></div></div></li>`
                        );
                    })
                } else
                    alert('Uploading error!');
