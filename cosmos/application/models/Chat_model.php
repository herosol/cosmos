...
class Chat_model extends CRUD_Model
{
    ...
    function get_chat_detail($chat_id)
    {
        ...
        foreach ($query->result() as $index => $row) {
            ...
            if($row->msg_type == 'booking')
                $row->booking = $this->get_admin_chat_booking($row->id);
            $rows[$index] = $row;
        }
        return $rows;
    }

    function get_admin_chat_booking($msg_id)
    {
        $this->db->where('msg_id', $msg_id);
        $this->db->limit(1);
        $query = $this->db->get('chat_bookings');
        return $query->row();
    }
    ...
    function get_chat_msgs($chat_id)
    {
        ...
        foreach ($query->result() as $index => $row) {
            $row->attachments = $this->get_attachments($row->id);
            if($row->msg_type == 'booking')
                $row->booking = $this->get_chat_booking($row->id, $this->session->mem_id);
            $rows[$index] = $row;
        }
        return $rows;
    }
    ...
    function get_chat_booking($msg_id, $mem_id)
    {
        $this->db->where('msg_id', $msg_id);
    ...
    function save_chat_booking($vals, $id = '')
    {
        $this->db->set($vals);
        if ($id != '') {
            $this->db->where('id', $id);
            $this->db->update('chat_bookings');
            return $id;
        } else {
            $this->db->insert('chat_bookings');
            return $this->db->insert_id();
        }
    }
