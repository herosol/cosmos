...
class Buyers extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->data['pageView'] = ADMIN . '/buyers';
        $this->data['rows'] = $this->member_model->get_rows(array('mem_type' => 'buyer'));
    ...
    function manage()
    {
        ...
        if ($this->input->post()) {
            ...
            setMsg('success', 'Buyer has been saved successfully.');
            redirect(ADMIN . '/buyers/manage/' . $this->uri->segment(4), 'refresh');
            exit;
        }
    ...
    function active()
    {
        ...
        setMsg('success', 'Buyer has been activated successfully.');
        redirect(ADMIN . '/buyers', 'refresh');
    }
    ...
    function inactive()
    {
        ...
        $vals['mem_status'] = '0';
        $this->member_model->save($vals, $mem_id);
        setMsg('success', 'Buyer has been deactivated successfully.');
        redirect(ADMIN . '/buyers', 'refresh');
    }
    ...
    // $this->remove_member_data($this->uri->segment(4));
        ...
        setMsg('success', 'Buyer has been deleted successfully.');
        redirect(ADMIN . '/buyers', 'refresh');
    }
    ...
    {
        $id = intval($id);
        if($this->data['member_row'] = $this->member_model->getMember($id,array('mem_type' => 'buyer'))) {
